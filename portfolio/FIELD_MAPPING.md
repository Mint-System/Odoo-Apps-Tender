...
This document maps the original Odoo Studio field names (with `x_` and `x_studio` prefixes) to the new clean field names used in the `portfolio` module.
...
The following field in `reference.reference` references `portfolio.solution` but is commented out due to dependency direction (portfolio depends on reference):
